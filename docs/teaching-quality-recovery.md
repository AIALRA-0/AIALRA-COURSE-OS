...
7. 已明确保留并纠正的来源错误不再被审计器当成教学错误
8. Harness 文件哈希统一把 CRLF 与 CR 换行规范为 LF；同一提交在 Windows 与 Linux 上必须得到同一快照。Harness 升级到 2.4.46，下一轮仍用同一四类页面验证

## 25. 第九次最新版策略多页试验

`2e02c5c` 与 Harness 2.4.46 上线后，新建候选 `material-version-e006f8aa-quality-v246-candidate`，计划 `65195836-5731-430e-8038-e8fbf5f07327` 继续生成第 4、7、12、16 页。计划总费用 0.082854 美元，平均每页 0.020714 美元，仍低于每页 0.06 美元硬上限；正式 release 未修改

- 第 4 页任务 `96d74d71-e096-4b94-993b-616a1a888386` 失败，费用 0.026650 美元。草稿已经用完整斯特林公式得到 $1000!\approx4.02\times10^{2567}$，也明确说明来源中的 $10^{2500}$ 相差约 67 个数量级；旧检查器却把同段用于对比围棋的 $10^{360}$ 错绑定为阶乘估值，并把“不能称为同一量级”的否定句当成肯定断言
- 第 7 页任务 `adab6b35-4fa0-4fd4-9754-ad646c7214b7` 失败，费用 0.009815 美元。供应商连续返回无法解析的结构化输出，任务保留真实 `MODEL_PROVIDER_OUTPUT_JSON_INVALID`，没有用本地内容或备用模型掩盖
- 第 12 页任务 `25438038-656f-45a5-a55e-946e98573b60` 失败，费用 0.027094 美元。草稿已经正确区分回合与时间步、宏单元智能体与标准单元基于力方法、固定线长系数与符号未知权重；旧检查器跨分句拼接条件，把这些正确限定再次误报
- 第 16 页任务 `18eedd35-097a-4bc5-842d-ec527e0795d9` 失败，费用 0.019295 美元。草稿明确说图输入不是固定长度向量，旧否定识别器没有覆盖“不是”这种自然表达，反而将其判成固定长度夸大

本轮修正仍然只调整通用语义和版式规则：

1. 阶乘指数只与同句中明确的等式、近似或估值关系绑定；远大于、相差和围棋量级比较不再被误认成阶乘估值
2. “不能称为同一量级”等明确否定不再触发错误；易错点中的错误理解先移除，再检查正确判断与解析
3. 回合与时间步的双向自然表达都能识别；标准单元区段只有实际把末次动作归给该阶段时才失败
4. 固定为负一的无权线长项与带符号权重项分别检查；当其他输入不变时，线长的确定方向不要求额外假设权重为正
5. “不是固定长度向量”属于明确否定；英文来源标签首次翻译后可以复用，受控术语表支持识别相邻中文翻译
6. 超过可读长度的中文段落只在句子边界确定性分段；代码、公式、表格、标题和列表保持原样，分段不引入写作策略禁止的中文句号或行尾分号
7. Harness 升级到 2.4.47；下一轮只重试这四个失败页，并继续执行机器检查、逐段核读和真实页面渲染验证

## 26. 第十次最新版策略多页试验

`806b0b1` 与 Harness 2.4.47 上线后，新建候选 `material-version-e006f8aa-quality-v247-candidate`，计划 `182a7def-8506-42b4-9f49-dba013cdf425` 继续生成第 4、7、12、16 页。计划总费用 0.056153 美元，平均每页 0.014038 美元；正式 release 未修改

- 第 4 页任务 `3c67cbaf-f0c5-486a-8c1d-e45576e6938b` 失败，费用 0.027471 美元。成稿已正确保留来源粗略值并复算准确阶乘量级，剩余误判来自检测器只识别“页面”而漏掉等价的“本页”来源限定词
- 第 7 页任务 `34a9f4a2-b447-471e-a7cb-66fede84c869` 失败，费用 0.014642 美元。公式、参数更新与更新后概率全部一致；唯一失败是 OCR 将 $\pi(a_1)=\pi(a_2)=0.5$ 识别成重复数学字符，合法 KaTeX 公式无法与旧引文做字符级匹配
- 第 12 页任务 `7f428937-a5f8-4b77-abc5-59a1ed03ce4c` 失败。正文格式和语义门已清零，来源审计连续两次把带显式乘号与省略乘号的同一加权公式判为不同形式，返回 `source_supported_formula_mismatch`
- 第 16 页任务 `1d59694f-2a20-4017-98af-faf38715b378` 因一次 `MODEL_PROVIDER_NETWORK_FAILURE` 失败，没有留下半完成草稿或隐式回退

本轮继续收紧证据而不是降低要求：

1. “本页把……写成……”与“页面把……写成……”使用同一来源限定规则；只要同字段明确标为粗略写法并给出准确复算，就不再把保留原式误判为作者错误
2. OCR 数学覆盖新增保守签名对账；只有同一当前行同时包含来源原子的全部变量、下标和数值时才能关闭覆盖，缺少任一变量或数值仍然失败
3. 来源审计的加权目标公式比较忽略 $\\times$、$\\cdot$、乘号与省略乘号的纯排版差异，仍严格比较负号、权重和每一项，缺项或权重错配继续失败
4. Harness 升级到 2.4.48；网络失败页与三个已定位页使用新快照重试，旧候选继续作为可复核失败证据
