...
export function ReviewWorkspace({ releases, reviewMap, onOpenPage, onReviewChanged }: {
  ...
  if (!reviewMap) return <div className="review-unavailable"><span><Icon name="warning" /></span><h1>掌握地图暂时不可用</h1><p>掌握地图尚未返回；已保存的自我重述卡片仍可独立复习</p><div className="review-unavailable-actions"><button className="primary-button" data-action="review-open-self-retelling-cards" onClick={() => setSelfRetellingReviewOpen(true)}>打开自我重述卡片</button><button className="quiet-button" data-action="review-reload-map" aria-describedby="review-reload-map-reason" disabled={busy} onClick={() => { setBusy(true); setError(""); Promise.resolve(onReviewChanged?.()).catch((reason) => setError(reason instanceof Error ? reason.message : "复习数据载入失败")).finally(() => setBusy(false)); }}>{busy ? "正在重试" : "重试掌握地图"}</button></div><span id="review-reload-map-reason" className="sr-only">{busy ? "正在重新读取掌握地图" : "重新读取当前正式课程的掌握记录"}</span>{error && <p className="review-error"><Icon name="warning" />{error}</p>}</div>;
