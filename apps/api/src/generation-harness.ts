...
export function modelInput(input: PromptInput): string | Array<{ role: "user"; content: Array<{ type: "input_text"; text: string } | { type: "input_image"; image_url: string; detail: "high" }> }> {
  ...
  const repairText = input.repair ? [
    ...
    ...(input.repair.issues.includes("TEACHING_OBJECTIVE_EXPLANATION_CONTRADICTION") ? ["学习目标与完整讲解必须一致。若来源给出完整函数和更新式，就用通用数学规则完成可复算推导并兑现目标；若确实缺少必要条件，就同步收窄目标。不得一边承诺算出结果，一边说结果无法确定；不得把输入项不同且关系未说明的公式称为等价，不得承诺解释未知拼接项的总维度，也不得在材料只展示布线结果时承诺说出布线方法"] : []),
