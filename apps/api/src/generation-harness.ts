...
export function modelInput(input: PromptInput): string | Array<{ role: "user"; content: Array<{ type: "input_text"; text: string } | { type: "input_image"; image_url: string; detail: "high" }> }> {
  ...
  const repairText = input.repair ? [
    ...
    ...(input.repair.issues.includes("TEACHING_SOURCE_COMMENTARY_OVERUSE") ? ["只改 fullExplanationMarkdown 的叙述视角。直接讲对象、关系、步骤和结论，不再让多数段落以‘页面给出’‘本页列出’‘表中写着’‘原图显示’开头。只有保留原始标签、指出来源冲突或说明证据边界时才提来源；压缩重复的来源旁白，不能删掉任何对象、数字、公式、条件、例子或必要边界"] : []),
