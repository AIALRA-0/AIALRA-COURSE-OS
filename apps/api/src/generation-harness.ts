...
export function modelInput(input: PromptInput): string | Array<{ role: "user"; content: Array<{ type: "input_text"; text: string } | { type: "input_image"; image_url: string; detail: "high" }> }> {
  ...
  const repairText = input.repair ? [
    ...
    ...(input.repair.issues.some((issue) => issue.startsWith("TEACHING_ABBREVIATION_PLACEMENT:")) ? ["只修复被指出字段中的中英文定义顺序：独立缩写放在中文全称之前，括号中只保留有对应依据的英文名称，例如‘MDP 马尔可夫决策过程（Markov Decision Process）’；正式名称内部已有缩写时保留原名并就近展开；不得写成‘马尔可夫决策过程（Markov Decision Process, MDP）’，也不得改动相关事实"] : []),
    ...
    ...(input.repair.issues.some((issue) => issue.includes("UNPAIRED_ENGLISH")) ? ["逐字段检查承上启下、目标、定义、完整讲解、总结、易错点和四道题的题干与答案解释。普通英文名称或缩写在首次定义后改用已核实的中文名称；首次按策略写成中文全称（有依据的英文名称），独立缩写放在中文名称之前；产品和组织核对官方名称，学术概念可核对可靠学术来源。原图中的坐标轴、图例、按钮和条目标签若保留英文，逐项用引号包住完整原文，并就近说明意义，连续列举也不能只给首项加引号；不得把原图标签当作无须解释的普通英语放在正文中。原图中的整段代码原样放进 Markdown 行内代码或代码块，再用中文说明用途；无法核实全称的原图标签用中文引出，并用引号保留原样，说明当前页能确认什么，不得猜测英文全称；原文论文标题用书名号保留。尤其不能让题目解释重新裸用正文已经解释过的缩写"] : []),
    ...(input.repair.issues.includes("TEACHING_PRIOR_DEFINITION_INCOMPLETE") ? ["逐条重写 priorKnowledge：每项只占一个列表项，格式为‘中文全称（有依据的英文名称）：是什么；具体做什么；怎样工作；何时使用；如何区分’，无法核对中英文对应时只保留中文名称。按内容选择三至五个完整分句，用中文分号隔开；每句至少十二字，不要把五种关系挤成逗号串，也不要编造英文名称"] : []),
    ...
    ...(input.repair.issues.includes("TEACHING_PRIOR_TERM_PAIR_MALFORMED") ? ["先验知识的术语名称中不能嵌套英文引号或拼接两个概念。括号里只保留一个有对应依据的英文名称本体；无法核实就删掉英文配对，只保留准确中文名称，不改动定义事实；名称本体内部确有缩写时保留原拼写并就近解释"] : []),
...
export function currentGenerationHarness(): GenerationHarnessSnapshot {
  const files = ["teaching-system-prompt.md", "teaching-user-prompt.md", "teaching-blueprint.md", "teaching-package.schema.json", "source-audit-prompt.md", "teaching-audit-prompt.md", "semantic-audit-prompt.md", "semantic-audit.schema.json", "policy-skill.md", "policy-format-rules.md", "policy-explanation-framework.md", "policy-formula-explanation.md"].map((name) => ({ path: name, sha256: generationHarnessFileSha256(readHarnessFile(name)) }));
  for (const name of ["page-plan-prompt.md", "planned-writing-prompt.md", "writing-format-contract.md"]) files.push({ path: name, sha256: generationHarnessFileSha256(readHarnessFile(name)) });
  // Only generation semantics belong in the immutable Harness snapshot. Runtime
  // orchestration in app.ts can change without invalidating in-flight content.
  for (const name of ["generation-harness.ts", "teaching-blueprint.ts", "model-router.ts", "teaching-patches.ts", "model-usage-meter.ts", "pricing.ts", "teaching-plan.ts", "planned-teaching.ts", "generation-repair.ts"]) {
