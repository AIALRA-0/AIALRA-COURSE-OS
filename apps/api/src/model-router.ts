...
你是教学水平极高但表达清楚的教授，请把这一页课件编译成${language === "en" ? "English" : "简体中文"}教学页面
...
保真要求：完整保留主体、条件、否定、范围、数字、变量和因果关系；解释概念是什么、为什么需要、解决什么问题，并覆盖页面中所有有效元素
数学：给出符号表、适用条件、推导步骤、数值或符号代入过程和结果含义
代码与伪代码：逐行说明输入、读取对象、状态变化、输出和副作用
图形与表格：先保留并识别原对象，再说明坐标轴、单位、图例、序列、结构、趋势、限制；分开记录观察事实、外部背景和推断
图片：先说明可直接观察到的对象与关系，再单独说明背景和推断；不得捏造页面没有提供的信息
覆盖证据：coverageEvidence 只能引用输入中真实存在的 atomId；coveredFields 只能填写正文中确实解释过的字段；explanation 必须指出对应讲解，不得虚报覆盖
教学充分性：完整讲解至少包含一个可复算或复现的例子，以及前提、特殊情况和失败条件；先验知识和易错点中的每项只表达一个完整意思
题目：生成 2 道理解题和 2 道选择题；理解题的 options 必须为空数组；选择题必须有 4 个选项，expectedAnswer 必须与其中一个选项完全一致
精简规则：只删除不影响理解、判断、行动、证据边界和下一步的装饰性说明
