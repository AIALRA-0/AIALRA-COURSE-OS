...
export class HttpProviderTeachingClient implements ModelRouterClient {
  ...
  async understandPage(input: ModelRouterInput) {
    ...
    const response = await this.requestPlannedStage(input, {
      ...
      instructions: "看清这一页课件，再为第一次接触本页知识的读者安排简短教学顺序。先忠实转写可见内容：列表逐项保留，表格写明行列标题并逐行转写数值，公式保留变量和关系；图中标签与箭头只有在能够沿唯一且清楚的线段准确追到两端对象时才写出对应关系。边相互交叉、标签邻近多条线或端点不清时，只描述图的结构和可见数字，不把数字绑定到某条边，也不与表格逐项比较；没有单位或所计对象的数字标签保留原文，不推定含义。不要根据提取文字的换行猜测项目数或表格值。密集矩阵不要逐格转写，只选少量能在原图中按行、列位置核实的代表值并注明位置；确实无法辨认的单元格标明不确定；两处确能独立定位的数值有差异时逐处保留，不能擅自统一，也不能把看不清的局部编造成冲突。只根据图片和辅助提取文字描述实际可见对象及关系；提取文字与原图冲突时按原图描述，不在教学输入里讨论识别差异；无法看清的局部直接标明不确定，不补成确定数字。页码、页眉和纯排版元素不成为教学对象。输出两个自然语言小段，分别以“页面内容：”和“教学顺序：”开头；不输出 JSON、来源编号或覆盖账本。",
  ...
  private async requestPlannedStage(input: ModelRouterInput, request: PlannedCall, budget: number) {
    ...
    // Chat-completions gateways may honor this OpenAI-compatible hint; sending it does not guarantee image-resolution quality.
