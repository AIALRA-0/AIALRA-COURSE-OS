...
export class HttpProviderTeachingClient implements ModelRouterClient {
  ...
  async understandPage(input: ModelRouterInput) {
    ...
    const response = await this.requestPlannedStage(input, {
      ...
      instructions: "看清这一页课件，再为第一次接触本页知识的读者安排简短教学顺序。先忠实转写可见内容：列表逐项保留；普通短表写明行列标题和可读数值；公式保留变量和关系。密集数值表或矩阵只说明行列含义，并最多选两个能同时按行标签、列标签和交叉位置核实的例值；不要枚举整行、整列或大量单元格。选出的每个例值还要核对对称位置或相邻标签，不能把相邻行、列的数字移过来；不能完成核对就不要写具体数值。计算必须包含所选范围内每个可见数值，无法逐个确认时不计算具体总和。图中标签与箭头只有在能够沿唯一且清楚的线段准确追到两端对象时才写出对应关系。边相互交叉、标签邻近多条线或端点不清时，只描述结构和可见数字，不把数字绑定到某条边；没有单位或所计对象的数字保留原文，不推定含义。不要根据提取文字的换行猜测项目数或表格值。只根据图片和辅助提取文字描述实际可见对象及关系；旧版讲解仅供参考，不能当作原图事实。图与辅助文字冲突时按图描述，不在教学输入里讨论识别差异；无法看清的局部标明不确定，不补成确定数字。页码、页眉、页脚和纯排版元素不成为教学对象。输出两个自然语言小段，分别以“页面内容：”和“教学顺序：”开头；不输出 JSON、来源编号或覆盖账本。",
